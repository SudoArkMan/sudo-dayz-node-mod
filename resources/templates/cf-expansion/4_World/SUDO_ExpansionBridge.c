// Everything that knows Expansion exists, kept in one place and behind guards.
//
...
{
	// Compile time. EXPANSIONMODCORE is defined in
	// 0_DayZExpansion_Core_Preload/Common/DayZExpansion_Core_Defines.c, right
	// under a comment saying it is published for third party mods that want to
	// know whether Expansion is loaded. Prefer it over the per feature flags
	// (EXPANSIONMODAI, EXPANSIONMODMARKET and the rest) unless you touch that
	// subsystem.
	//
	// It reaches your script because the preload registers its Common folder
	// into all five script modules, and because a define is visible only to
	// files compiled after it and the 0_ prefix sorts that PBO ahead of
	// anything starting with a letter. That ordering is filename sort and
	// nothing else: the preload declares requiredAddons[] = {}, so no
	// dependency edge enforces it.
	...
	// Runtime. DZ_Expansion_Core_Preload is the CfgMods class name of that same
	// empty preload PBO, which makes it a capability beacon anything can test
	// for. Expansion uses precisely this shape on itself.
	static bool ExpansionIsLoaded()
	{
		return GetGame().ConfigIsExisting("CfgMods DZ_Expansion_Core_Preload");
	}

	// One method per Expansion feature you touch, shaped like this one: vanilla
	// types in the signature, the guard inside the body, and a real answer on
	// the #else branch rather than a crash.
